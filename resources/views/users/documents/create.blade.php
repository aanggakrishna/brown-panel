...
<div class="bg-light rounded">
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <h6 class="card-subtitle text-muted">Upload document for {{ $user->name }}</h6>
                </div>
        ...
        <div class="card-body">
            <form action="{{ route('user-documents.store', $user) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select name="document_type" id="document_type" class="form-select @error('document_type') is-invalid @enderror" required>
                                ...
                                <option value="ktp" {{ old('document_type') == 'ktp' ? 'selected' : '' }}>KTP (Identity Card)</option>
                                <option value="npwp" {{ old('document_type') == 'npwp' ? 'selected' : '' }}>NPWP (Tax ID)</option>
                                <option value="bpjs_kesehatan" {{ old('document_type') == 'bpjs_kesehatan' ? 'selected' : '' }}>BPJS Kesehatan</option>
                                <option value="bpjs_tenaga_kerja" {{ old('document_type') == 'bpjs_tenaga_kerja' ? 'selected' : '' }}>BPJS Tenaga Kerja</option>
                                <option value="ijazah" {{ old('document_type') == 'ijazah' ? 'selected' : '' }}>Ijazah (Diploma)</option>
                                <option value="cv" {{ old('document_type') == 'cv' ? 'selected' : '' }}>CV/Resume</option>
                                <option value="photo" {{ old('document_type') == 'photo' ? 'selected' : '' }}>Photo</option>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" name="document_name" id="document_name" class="form-control @error('document_name') is-invalid @enderror"
                                   value="{{ old('document_name') }}" placeholder="e.g., KTP - John Doe" required>
                ...
                <div class="d-flex justify-content-end">
                    ...
                    <button type="submit" class="btn btn-primary-gradient">
                        <i class="cil-cloud-upload me-1"></i> Upload Document
                    </button>
                </div>
            </form>
        </div>
