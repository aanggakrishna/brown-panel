                    ...
                    <form action="{{ route('leave-applications.update', $leaveApplication) }}" method="POST">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="date" class="form-control @error('start_date') is-invalid @enderror" id="start_date" name="start_date"
                                           value="{{ old('start_date', $leaveApplication->start_date->format('Y-m-d')) }}" min="{{ date('Y-m-d') }}" required>
                                    @error('start_date')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="date" class="form-control @error('end_date') is-invalid @enderror" id="end_date" name="end_date"
                                           value="{{ old('end_date', $leaveApplication->end_date->format('Y-m-d')) }}" min="{{ date('Y-m-d') }}" required>
                                    @error('end_date')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="total_days" value="{{ $leaveApplication->total_days }} days" readonly>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('reason') is-invalid @enderror" id="reason" name="reason" rows="3"
                                      placeholder="Please provide a detailed reason for your leave application" required>{{ old('reason', $leaveApplication->reason) }}</textarea>
                            @error('reason')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="2"
                                      placeholder="Any additional information or special requests">{{ old('notes', $leaveApplication->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="text-center">
                        <h2 class="text-primary">{{ $currentBalance + $leaveApplication->total_days }}</h2>
                        <p class="text-muted mb-0">Days Remaining</p>
                        <small class="text-muted">For {{ date('F Y') }} (including current application)</small>
                    </div>
                </div>
            </div>
            ...
            <div class="card mt-3">
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center mb-2">
                        <span class="badge {{ $leaveApplication->status === 'pending' ? 'bg-warning' : ($leaveApplication->status === 'approved' ? 'bg-success' : 'bg-danger') }} me-2">
                            {{ ucfirst($leaveApplication->status) }}
                        </span>
                        <small class="text-muted">Applied on {{ $leaveApplication->created_at->format('M d, Y') }}</small>
                    </div>
                    @if($leaveApplication->approved_at)
                        <small class="text-muted">
                            {{ $leaveApplication->status === 'approved' ? 'Approved' : 'Rejected' }} on {{ $leaveApplication->approved_at->format('M d, Y') }}
                        </small>
                    @endif
                </div>
            </div>
